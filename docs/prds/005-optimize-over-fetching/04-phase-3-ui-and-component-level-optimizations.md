...
-   **Problem:** Queries that populate the project board (`listProjectTasksWithRelations`, `fetchProjectRelations`) fetch the full metadata for every attachment on every task. The `TaskCard` component only displays the *count* of attachments.
