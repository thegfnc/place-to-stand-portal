...
- Converted Clients surfaces to use new Drizzle queries (`getClientsSettingsSnapshot`, slug uniqueness helpers, dependency counters) and guard helpers:
  - `app/(dashboard)/clients/page.tsx` and the nested archive/activity routes now pull clients, memberships, and client directory data through `lib/queries/clients.ts`.
...
- Load `/clients` (and the `/clients/archive` + `/clients/activity` views) as an admin and confirm client lists, members, and directory data render correctly.
