...
- **As a Client, I want to** view all my work organized on a Kanban board in the work > projects menu, **so that I** can easily track the status of every task.
...
- **Projects Menu:**
  ...
  - **Permissions:** Clients can comment on existing tasks. They cannot change a task's status (i.e., drag and drop cards between columns).
