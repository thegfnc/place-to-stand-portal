    ...
    if (normalizedAvatarPath) {
      try {
        await deleteAvatarObject({
          client: adminClient,
          path: normalizedAvatarPath,
        })
      } catch (cleanupError) {
        console.error(
          'Failed to clean up avatar after profile insert failure',
          cleanupError
        )
      }
    }
  ...
  if (normalizedAvatarPath) {
    ...
    if (metadataResult.error) {
      ...
      try {
        ...
        console.error(
          'Failed to clean up avatar after metadata failure',
          cleanupError
        )
      }
  ...
  try {
    ...
    if (normalizedAvatarPath) {
      try {
        ...
        console.error(
          'Failed to clean up avatar after invite failure',
          cleanupError
        )
      }
    }
...
export async function updateUser(
  ...
  const { id, fullName, role, password, avatarPath, avatarRemoved } =
    parsed.data
  const adminClient = getSupabaseServiceClient()
