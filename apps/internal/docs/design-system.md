...
1. **Content-sized columns get a content floor with a proportional share: `w-[max(<rem>,<percent>)]`.** Badges, toggles, icon-plus-count cells, dates, avatars, currency or hour figures. The `rem` is the widest realistic content plus cell padding (a lone badge `5rem`–`6rem`, a toggle with its label `7rem`, an icon-plus-count `4rem`–`5rem`, a short date `7rem`, "0 active (2 total)" `9rem`); the percent is that width's share of a ~1200px table (`px / 1200`), so the column sits at its floor on a laptop and grows proportionally on a wide monitor. Fixed layout ignores `min-width` on cells but honours `max()` inside `width`, which is what makes this work. The Actions column and icon-only columns keep a plain fixed width (`w-24`, `w-6`). A bare percentage on a narrow column is what left empty air beside a single icon on small screens.
...
3. **Hide columns by tier so the fixed columns fit a budget.** The flexible text columns only get what the fixed columns leave over, and fixed layout ignores `min-width`, so when the fixed widths outgrow the table the identity column collapses to zero and neighbours paint over it. Budget the *visible* fixed columns per breakpoint: about **200px below `md`** (a ~330px phone table) and about **480px between `md` and `xl`** (the table is only ~750px at a 1024px viewport with the sidebar open, so the tertiary tier waits for `xl`, not `lg`). Put `hidden md:table-cell` on secondary columns and `hidden xl:table-cell` on tertiary ones, on the `TableHead` *and* on every `TableCell` of that column. Typical phone survivors: identity, one key badge or figure, Actions. Never hide the identity column or Actions. Full-width rows (empty states, group separators) must span only the *visible* columns: under fixed layout a `colSpan` wider than the visible header is not clamped — the browser adds anonymous columns and hands them the leftover width. Use `FullWidthCell` from `components/table-toolbar/full-width-cell.tsx` with the per-tier counts instead of a literal `colSpan`.
