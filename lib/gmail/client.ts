...
type GetAccessTokenResult = { accessToken: string; expiresAt?: Date | null }
...
export async function getValidAccessToken(userId: string): Promise<GetAccessTokenResult> {
  ...
  if (!needsRefresh) {
    return { accessToken: decryptToken(conn.accessToken), expiresAt: exp ? new Date(exp) : null }
  }
  ...
  return { accessToken: refreshed.access_token, expiresAt: new Date(newExpiresAt) }
}

export async function listMessages(userId: string, params?: { maxResults?: number; pageToken?: string; q?: string }) {
